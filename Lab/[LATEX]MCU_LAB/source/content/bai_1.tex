...
\textbf{Report 1: } The Proteus schematic contains two LEDs connected to GPIO output pins of the STM32. In this implementation, the red LED is connected to \textbf{PA10} and the yellow LED is connected to \textbf{PA11}. Both LEDs are connected in active-low mode: the positive terminal of each LED is connected to the 3.3V supply through a current-limiting resistor, while the negative terminal is connected to the GPIO pin. Therefore, writing logic 0 to the GPIO pin turns the LED on, and writing logic 1 turns the LED off.\\

\textit{TODO: Add the Proteus schematic image and the project link after the schematic is finalized.}\\
